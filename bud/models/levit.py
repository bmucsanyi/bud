...
from bud.layers import DropPath, get_act_layer, to_2tuple, to_ntuple, trunc_normal_
...
class Levit(nn.Module):
    ...
    def reset_classifier(self, num_classes, global_pool=None, distillation=None):
        ...
        if global_pool is not None:
            self.global_pool = global_pool
        self.head = (
            NormLinear(self.embed_dim[-1], num_classes, drop=self.drop_rate)
            if num_classes > 0
            else nn.Identity()
        )
...
def create_levit(variant, cfg_variant=None, pretrained=False, distilled=True, **kwargs):
    ...
    if kwargs.get("features_only", None):
        if not is_conv:
            raise RuntimeError(
                "features_only not implemented for LeVit in non-convolutional mode."
            )
