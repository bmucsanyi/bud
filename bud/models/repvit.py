...
from bud.layers import SqueezeExcite, to_2tuple, to_ntuple, trunc_normal_
...
class RepVitDownsample(nn.Module):
    def __init__(self, in_dim, mlp_ratio, out_dim, kernel_size, act_layer):
        ...
        self.pre_block = RepViTBlock(
            in_dim, mlp_ratio, kernel_size, use_se=False, act_layer=act_layer
        )
    ...
    def __init__(
        ...
        if downsample:
            self.downsample = RepVitDownsample(
                in_dim, mlp_ratio, out_dim, kernel_size, act_layer
            )
        else:
        ...
        for _ in range(depth):
            blocks.append(
                RepViTBlock(out_dim, mlp_ratio, kernel_size, use_se, act_layer)
            )
            use_se = not use_se
