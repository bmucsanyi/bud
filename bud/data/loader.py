...
from .dataset import IterableImageDataset, SoftImageNet
...
def fast_collate(batch):
    ...
    if isinstance(batch[0][0], tuple):
        ...
        # flatten them into the format [pos0, pos0, pos1, pos1, ...] to make it
        # compatible to normal training and eval
        # The contrastive losses will take care of casting them back to
        # [[pos0, pos0], [pos1, pos1], ...]
        num_positive = len(batch[0][0])
        targets = torch.from_numpy(
            np.array([[b[1] for _ in range(num_positive)] for b in batch])
        ).flatten()
        assert len(targets) == batch_size * num_positive
        tensor = torch.zeros(
            (batch_size * num_positive, *batch[0][0][0].shape), dtype=torch.uint8
        )
        for i in range(batch_size):
            for p in range(num_positive):
                tensor[i * num_positive + p] += batch[i][0][p]
        return tensor, targets
    else:
        assert False


def contrastive_tensor_collate(batch):
    num_positive = len(batch[0][0])  # Number of positive samples
    batch_size = len(batch)  # Batch size

    tensor_shape = batch[0][0][0].shape  # Shape of individual tensor
    tensor_dtype = batch[0][0][0].dtype
    label_dtype = torch.int64
    # Shape of all_tensors: [batch_size * num_positive, *tensor_shape]
    all_tensors = torch.zeros(
        (batch_size * num_positive, *tensor_shape), dtype=tensor_dtype
    )
    # Shape of all_labels: [batch_size * num_positive]
    all_labels = torch.zeros(batch_size * num_positive, dtype=label_dtype)

    for i, (tensors, label) in enumerate(batch):
        all_tensors[i * num_positive : (i + 1) * num_positive] = torch.stack(tensors)
        all_labels[i * num_positive : (i + 1) * num_positive] = label
...
def create_loader(
    ...
    if prepare_n_crop_transform:
        dataset_transform = prepare_n_crop_transform(
            [dataset_transform], num_crops_per_aug=[2]
        )
    ...
    if collate_fn is None:
        ...
        elif prepare_n_crop_transform:
            collate_fn = contrastive_tensor_collate
        else:
