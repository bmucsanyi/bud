...
class ReaderCUB(Reader):
    def __init__(self, root, split="test", **kwargs):
        ...
        image_list = [
            [(key, img_path) for img_path in image_list[key]]
        ...
        # Use the first half of the sorted data as training and the second half as test
        # set
        ...
        if split != "test":
            ...
            for key in train:
                    ...
                    list(
                        set(
                            np.round(
                                np.linspace(
                                    0,
                                    len(image_dict[key]) - 1,
                                    int(len(image_dict[key]) * 0.5),
                                )
                            )
                        )
                    )
                ...
                val_ixs = np.array(
                    [x for x in range(len(image_dict[key])) if x not in train_ixs]
                )
        ...
        # Flatten all classes into one dict
        self.samples = [
            [(x, key) for x in self.samples[key]] for key in self.samples.keys()
        ]
    ...
    def __getitem__(self, index):
        ...
        return pil_loader(path), torch.tensor(label)
...
class ReaderSOP(Reader):
    def __init__(self, root, split="test", **kwargs):
        ...
        training_files = pd.read_table(
            root + "/Info_Files/Ebay_train.txt", header=0, delimiter=" "
        )
        test_files = pd.read_table(
            root + "/Info_Files/Ebay_test.txt", header=0, delimiter=" "
        )
        ...
        if split != "test":
            ...
            for super_ix in super_dict.keys():
                ...
                for _, class_ix in enumerate(class_ixs[:train_val_split]):
                    train_image_dict[super_ix][train_count] = super_dict[super_ix][
                        class_ix
                    ]
                    train_count += 1
            ...
            for super_ix, super_set in train_image_dict.items():
                ...
                for class_ix, class_set in super_set.items():
                    ...
                    if class_ix not in train_conversion:
                        train_conversion[class_ix] = (
                            class_set[0].split("/")[-1].split("_")[0]
                        )
