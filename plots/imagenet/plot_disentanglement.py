...
def main(args):
    ...
    for prefix in dataset_conversion_dict:
        ...
        for method_id, method_name in tqdm(id_to_method.items()):
            if method_name not in POSTERIOR_ESTIMATORS: continue
            ...
            if aggregated_key is None:
                ...
                means = {
                    key: np.mean(metric[key])
                    for key in metric
                    if r"\text{B}" not in key
                }
            ...
            try:
                method_name = (
                    method_name if default_key else f"{method_name} {aggregated_key}"
                )
