...
def main():
    ...
    create_directory(f"results/corr_vs_acc_top5")
    ...
    for method_id, method_name in tqdm(id_to_method.items()):
        ...
        auroc_correctness_matrix = np.zeros((num_successful_runs, 6))  # [runs, severities]
        ...
        save_path = f"results/corr_vs_acc_top5/{method_name}.pdf"
        ...
        plt.errorbar(
            ...
            color=np.array([52.0, 168.0, 83.0]) / 255.0,
            markersize=3,
        ...
        plt.errorbar(
            ...
            color=np.array([251.0, 188.0, 4.0]) / 255.0,
            markersize=3,
            ecolor=np.array([105.0, 109.0, 113.0]) / 255.0,
            elinewidth=1,
            capsize=5,
        ...
        plt.errorbar(
            ...
            fmt="-o",
            label="Accuracy",
            color=np.array([66.0, 103.0, 210.0]) / 255.0,
            markersize=3,
            ecolor=np.array([105.0, 109.0, 113.0]) / 255.0,
            elinewidth=1,
            capsize=5,
        ...
        plt.ylabel(rf"{method_name} AUROCs and Acc. $\uparrow$")
        plt.ylim(0, 1)
