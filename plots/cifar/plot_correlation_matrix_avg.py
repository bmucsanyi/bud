...
def main():
    ...
    performance_matrix = np.zeros((len(metric_dict), len(id_to_method)))
    ...
    for distributional_estimator in ["one_minus_max_probs_of_fbar", "one_minus_max_probs_of_bma", "one_minus_expected_max_probs"]:
        for j, (method_id, method_name) in enumerate(tqdm(id_to_method.items())):
            ...
            for i, (metric_id, metric_name) in enumerate(metric_dict.items()):
                ...
                performance_matrix[i, j] = estimator_dict[estimator]
